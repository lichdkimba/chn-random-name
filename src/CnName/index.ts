...
const generate_CN_random_name = (config: NameProps): NameType => {
  ...
  if (config.Gender === 'Female') {
    // 决定单字名或双字名
    if (Math.random() >= 0.5) {
      let temp_random = Math.floor(Math.random() * CnSingleGivenNameFemale.length)
      GivenName = CnSingleGivenNameFemale[temp_random]
    }else {
      let temp_random = Math.floor(Math.random() * CnGivenNameFemale.length)
      GivenName = CnGivenNameFemale[temp_random]
    }
  }
  if (config.Gender === 'Male') {
    if (Math.random() >= 0.5) {
      let temp_random = Math.floor(Math.random() * CnSingleGivenNameMale.length)
      GivenName = CnSingleGivenNameMale[temp_random]
    }else {
      let temp_random = Math.floor(Math.random() * CnGivenNameMale.length)
